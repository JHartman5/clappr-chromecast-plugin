(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ChromecastPlugin"] = factory();
	else
		root["ChromecastPlugin"] = factory();
})(this, function() {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _clapprCore = __webpack_require__(1);
	...
	var ChromecastPlugin = (function (_UICorePlugin) {
	  ...
	  _createClass(ChromecastPlugin, [{
	    ...
	    value: function bindEvents() {
	      this.listenTo(this.core.mediaControl, _clapprCore.Events.MEDIACONTROL_RENDERED, this.render);

	      if (_clapprCore.Events.CORE_ACTIVE_CONTAINER_CHANGED) {
	        // Clappr 0.3.0 or greater
	        this.listenTo(this.core, _clapprCore.Events.CORE_ACTIVE_CONTAINER_CHANGED, this.containerChanged);
	      } else {
	        this.listenTo(this.core.mediaControl, _clapprCore.Events.MEDIACONTROL_CONTAINERCHANGED, this.containerChanged);
	      }

	      if (this.container) {
	        this.listenTo(this.container, _clapprCore.Events.CONTAINER_TIMEUPDATE, this.containerTimeUpdate);
	        this.listenTo(this.container, _clapprCore.Events.CONTAINER_PLAY, this.containerPlay);
	        this.listenTo(this.container, _clapprCore.Events.CONTAINER_ENDED, this.sessionStopped);
	      }
	    }
	  }, {
	    key: "isBootable",
	    value: function isBootable() {
	      ...
	      if (!_clapprCore.Browser.isChrome) {
	        return false;
	      }

	      // Chrome lesser than or equals to 71
	      // does not require secure page
	      if (_clapprCore.Browser.version <= 71) {
	        return true;
	      }
	    ...
	    value: function bootstrapCastApi() {
	      ...
	      if (this.bootTryCount > this.bootMaxTryCount) {
	        this.bootTryCount = 0;
	        _clapprCore.Log.warn("GCastApi bootstrap timeout");
	        this.disable();
	        return;
	      }
	        ...
	        if (window.chrome.cast && window.chrome.cast.isAvailable) {
	          ...
	          window["__onGCastApiAvailable"] = function (loaded, errorInfo) {
	            if (loaded) {
	              ...
	              _clapprCore.Log.warn("GCastApi error", errorInfo);
	              _this2.disable();
	            }
	          };
	        }
	    ...
	    value: function updateCCTrackID(trackID) {
	      ...
	      if (container) {
	        container.trigger(_clapprCore.Events.CONTAINER_SUBTITLE_CHANGED, { id: trackID });
	      }
	    }
	  }, {
	    ...
	    value: function initializeCastApi() {
	      ...
	      chrome.cast.initialize(apiConfig, function () {
	        return _clapprCore.Log.debug(_this3.name, "init success");
	      }, function () {
	        return _clapprCore.Log.warn(_this3.name, "init error");
	      });
	    }
	  }, {
	    key: "sessionListener",
	    value: function sessionListener(session) {
	      _clapprCore.Log.debug(this.name, "new session id:" + session.sessionId);
	    ...
	    value: function sessionUpdateListener() {
	      if (this.session) {
	        _clapprCore.Log.debug(this.name, this.session.status);
	    ...
	    value: function onSessionTextTracks(tracks) {
	      ...
	      if (this.textTracks.length > 0) {
	        ...
	        this.trigger(_clapprCore.Events.PLAYBACK_SUBTITLE_AVAILABLE);
	    ...
	    value: function receiverListener(e) {
	      if (e === chrome.cast.ReceiverAvailability.AVAILABLE) {
	        _clapprCore.Log.debug(this.name, "receiver found");
	        this.show();
	      } else {
	        _clapprCore.Log.debug(this.name, "receiver list empty");
	        this.hide();
	      }
	    }
	  }, {
	    ...
	    value: function launchSuccess(session) {
	      ...
	      _clapprCore.Log.debug(this.name, "launch success - session: " + session.sessionId);
	      this.newSession(session);
	    }
	  }, {
	    key: "launchError",
	    value: function launchError(e) {
	      _clapprCore.Log.debug(this.name, "error on launch", e);
	    ...
	    value: function loadMediaSuccess(how, mediaSession) {
	      ...
	      _clapprCore.Log.debug(this.name, "new media session", mediaSession, "(", how, ")");
	    ...
	    value: function loadMediaError(e) {
	      _clapprCore.Log.warn(this.name, "media error", e);
	    }
	  }, {
	    key: "newSession",
	    ...
	    value: function sessionStopped() {
	      ...
	      if (this.playbackProxy) {
	        if (this.playbackProxy.isPlaying() || playerState === "PAUSED") {
	          container.once(_clapprCore.Events.CONTAINER_READY, function () {
	            container.play();
	    ...
	    value: function loadMedia() {
	      ...
	      _clapprCore.Log.debug(this.name, "loading... " + src);
	    ...
	    value: function containerPlay() {
	      if (this.session && (!this.mediaSession || this.mediaSession.playerState === "IDLE" || this.mediaSession.playerState === "PAUSED")) {
	        _clapprCore.Log.debug(this.name, "load media");
	        this.loadMedia();
	      }
	    }
	  }, {
	    ...
	    value: function render() {
	      ...
	      this.$style = _clapprCore.Styler.getStyleFor(_publicStyleScss2["default"], {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var ChromecastPlayback = (function (_Playback) {
	  ...
	  _createClass(ChromecastPlayback, [{
	    ...
	    key: "template",
	    get: function get() {
	      return (0, _clapprCore.template)(_publicChromecastHtml2["default"]);
	    }
	  }, {
	    key: "attributes",
	    get: function get() {
	  ...
	  _createClass(ChromecastPlayback, [{
	    ...
	    value: function pause() {
	      ...
	      if (this.getPlaybackType() === _clapprCore.Playback.LIVE) {
	        this.trigger(_clapprCore.Events.PLAYBACK_DVR, true);
	      }
	    }
	  }, {
	    key: "stop",
	    ...
	    value: function seek(time) {
	      ...
	      this.currentMedia.seek(request, function () {
	        ...
	        return _clapprCore.Log.warn("seek failed");
	      });
	      if (this.getPlaybackType() === _clapprCore.Playback.LIVE) {
	        // assume live if time within 30 seconds of end of live stream
	        this.trigger(_clapprCore.Events.PLAYBACK_DVR, time < this.getDuration() - 30);
	      }
	    }
	  }, {
	    key: "seekPercentage",
	    ...
	    key: "getPlaybackType",
	    value: function getPlaybackType() {
	      return !!this.currentMedia.liveSeekableRange ? _clapprCore.Playback.LIVE : _clapprCore.Playback.VOD;
	    }
	  }, {
	    ...
	    value: function onMediaStatusUpdate() {
	      ...
	      if (this.currentMedia.playerState === "BUFFERING") {
	        ...
	        this.trigger(_clapprCore.Events.PLAYBACK_BUFFERING, this.name);
	      } else if (this.currentMedia.playerState === "PLAYING") {
	        if (this.isBuffering) {
	          this.isBuffering = false;
	          this.trigger(_clapprCore.Events.PLAYBACK_BUFFERFULL, this.name);
	        }
	        if (this.prevState !== this.currentMedia.playerState) {
	          this.trigger(_clapprCore.Events.PLAYBACK_PLAY, this.name);
	        }
	        ...
	        this.trigger(_clapprCore.Events.PLAYBACK_ENDED, this.name);
	      } else if (this.currentMedia.playerState === "PAUSED") {
	        if (this.prevState !== this.currentMedia.playerState) {
	          this.trigger(_clapprCore.Events.PLAYBACK_PAUSE, this.name);
	        }
	      }
	    ...
	    value: function updateMediaControl() {
	      ...
	      this.trigger(_clapprCore.Events.PLAYBACK_TIMEUPDATE, { current: position, total: duration }, this.name);
	    }
	  }, {
	    key: "show",
	    value: function show() {
