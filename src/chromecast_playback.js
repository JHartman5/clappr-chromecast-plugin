import { Events, Log, Playback, template } from "@clappr/core";
...
export default class ChromecastPlayback extends Playback {
  ...
  get template() {
    return template(chromecastHTML);
  }
  get attributes() {
  ...
  constructor(options) {
    ...
    this.settings.default &&
      (this.settings.default = this.settings.default.filter(noVolume));
    this.settings.left &&
      (this.settings.left = this.settings.left.filter(noVolume));
    this.settings.right &&
      (this.settings.right = this.settings.right.filter(noVolume));
  ...
  render() {
    ...
    if (this.options.poster) {
      this.$(".chromecast-playback-background").css(
        "background-image",
  ...
  isPlaying() {
    return (
      ...
      this.currentMedia.playerState === "BUFFERING"
    );
  }
